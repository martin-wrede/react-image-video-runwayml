// --- START OF FILE App.jsx (Final Working Version) ---
    ...
    pollIntervalRef.current = setInterval(async () => {
      try {
        ...
        if (!statusResponse.ok) {
           // Don't stop polling for minor server errors, just log it
          console.error(`Polling HTTP error! status: ${statusResponse.status}`);
          return;
        }
        ...
        // The backend sends progress as 0-1, so we convert to percentage
  ...
  const generateVideo = async () => {
    ...
    try {
      ...
      setTimeout(() => {
        if (pollIntervalRef.current) {
          clearInterval(pollIntervalRef.current);
          // Check if it's still generating after timeout
          if (videoUrl === null && !error) {
  ...
  return (
    <div style={{ padding: '20px', maxWidth: '600px', margin: '0 auto', fontFamily: 'sans-serif' }}>
      ...
      <div style={{ marginBottom: '20px' }}>
        ...
        <input
          type="text"
          placeholder="e.g., 'camera slowly zooms in', 'gentle wind'"
          value={prompt}
      ...
      <div style={{ marginBottom: '20px' }}>
        ...
        <input id="fileInput" type="file" accept="image/*" onChange={handleFileSelect} style={{ width: '100%' }} />
      </div>
      ...
      <button 
        ...
        style={{ padding: '10px 20px', fontSize: '16px', backgroundColor: (isGenerating || !selectedFile || !prompt.trim()) ? '#ccc' : '#007bff', color: 'white', border: 'none', cursor: 'pointer' }}
      >
        ...
        <div style={{ marginTop: '20px', padding: '10px', backgroundColor: '#f0f0f0' }}>
          ...
          {isGenerating && progress > 0 && (
            <div style={{ backgroundColor: '#ddd' }}>
              <div style={{ width: `${progress}%`, height: '20px', backgroundColor: '#007bff', transition: 'width 0.5s ease' }} />
            </div>
          )}
        </div>
      )}
      ...
      {videoUrl && (
       ...
       <div style={{ marginTop: '30px', fontSize: '14px', color: '#666' }}>
        <p><strong>Model:</strong> Gen-2 (Image-to-Video)</p>
